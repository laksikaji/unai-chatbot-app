...
serve(async (req: any) => {
    ...
    try {
        ...
        const recordsToProcess = dataRows.map((values) => {
            const record: any = {
                category: getValueByHeaders(values, [
                    'category', 'หมวดหมู่', 'ประเภทหลัก'
                ]),
                subcategory: getValueByHeaders(values, [
                    'subcategory', 'หมวดหมู่ย่อย', 'หมวดหมู่ ย่อย', 'ประเภท'
                ]),
                symptom_description: getValueByHeaders(values, [
                    'symptom', 'symptom_description', 'อาการ'
                ]),
                observation: getValueByHeaders(values, [
                    'observation', 'ข้อสังเกต', 'ข้อสังเกตุ', 'สังเกต', 'สังเกตุ'
                ]),
                initial_check: getValueByHeaders(values, [
                    'initial check', 'initial_check', 'ตรวจสอบเบื้องต้น'
                ]),
                possible_causes: getValueByHeaders(values, [
                    'possible causes', 'possible_causes', 'สาเหตุ',
                    'สาเหตุที่อาจเป็นไปได้'
                ]),
                solution: getValueByHeaders(values, [
                    'solution', 'วิธีแก้'
                ]),
                responsible_party: getValueByHeaders(values, [
                    'responsible', 'responsible_party', 'ผู้รับผิดชอบ',
