...
serve(async (req: any) => {
    ...
    try {
        ...
        console.log('=== PROCESSING GENERAL INFO FILE ===');
        ...
        const recordsToProcess = dataRows.map((values) => {
            const record: any = {
                title: getValueByHeaders(values, ['title', 'หัวข้อ', 'ชื่อ']),
                content: getValueByHeaders(values, ['content', 'เนื้อหา', 'รายละเอียด', 'คำตอบ']),
            ...
            record.embeddingContent = [
                `หัวข้อ: ${record.title}`,
                `เนื้อหา: ${record.content}`,
            ].join('\n');

            record.search_keywords = [record.title, record.content]
                .filter(Boolean).join(' ');
        ...
        for (const record of recordsToProcess) {
            const isDuplicate = existingRecords && existingRecords.some((ex: any) =>
                (String(ex.title || '').trim() === String(record.title || '').trim()) &&
                (String(ex.content || '').trim() === String(record.content || '').trim())
            );
        ...
        if (recordsWithEmbeddings.length > 0) {
            const { error: insertError } = await supabase
                .from('general_information').insert(recordsWithEmbeddings);
